	...
	private void writeContacts(String host, Vector<Contact> contacts) throws IOException {
		...
		if (!(writeList.isEmpty() && deleteList.isEmpty())) {
			if (confirmLists(writeList, deleteList)) {
				JOptionPane.showMessageDialog(null, _("<html>Scanning, merging and cleaning was <i>demonstrated</i>! To actually perform this tasks, purchase the full version or get the code from github and compile it yourself. Goodbye!"));
			} else {
